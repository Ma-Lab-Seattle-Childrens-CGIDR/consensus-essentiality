...
import pandas as pd
# Local imports
from consensus_essentiality.expression_utils import count_to_rpkm, count_to_fpkm, count_to_tpm, rpkm_to_tpm, \
    fpkm_to_tpm, count_to_cpm
...
class TestExpressionUtils(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.count_data = pd.DataFrame(count_values, index=sample_list, columns=gene_list)
        ...
        cls.small_feature_length = pd.Series([10, 50, 25, 60], index=["A", "B", "C", "D"])
    ...
    def test_count_to_rpkm(self):
        ...
        known_rpkm = pd.DataFrame({
            "A": [3.860465e+07, 2.500000e+07, 3.571429e+07, 8.900524e+06, 3.395522e+07],
            "B": [1.860465e+06, 6.000000e+05, 1.932773e+06, 4.712042e+06, 5.597015e+06],
            "C": [1.004651e+07, 1.300000e+07, 8.907563e+06, 2.031414e+07, 5.671642e+06],
            "D": [4.496124e+06, 6.583333e+06, 5.392157e+06, 2.792321e+06, 3.980100e+06],
        })
    ...
    def test_count_to_fpkm(self):
        ...
        """
        # Since this is just a wrapper, just test that it returns the same as count_to_rpkm
    ...
    def test_count_to_tpm(self):
        ...
        known_tpm = pd.DataFrame({
            "A": [701803.833145, 553301.364810, 687516.850903, 242395.437262, 690091.001011],
            "B": [33821.871477, 13279.232755, 37206.794284, 128326.996198, 113751.263903],
            "C": [182638.105975, 287716.709701, 171474.791049, 553231.939163, 115267.947422],
            "D": [81736.189402, 145702.692733, 103801.563764, 76045.627376, 80889.787664],
        })
        self.assertTrue(np.all(np.isclose(known_tpm, count_to_tpm(self.small_counts, self.small_feature_length))))
    ...
    def test_fpkm_to_tpm(self):
        ...
        self.assertTrue(np.all(np.isclose(tpm_fpkm, tpm_fpkm)))
